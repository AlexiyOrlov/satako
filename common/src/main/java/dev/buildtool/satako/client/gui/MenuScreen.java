...
public abstract class MenuScreen<T extends AbstractContainerMenu> extends AbstractContainerScreen<T> {
    ...
    @Override
    public void init() {
        int maxX = 0, minX = imageWidth, maxY = 0;
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float p_render_3_) {
        ...
        slots.stream().filter(Slot::isActive).forEach(slot -> Services.PLATFORM.drawSlot(slot, mouseX, mouseY, guiGraphics, this));
    ...
    protected void renderBg(GuiGraphics guiGraphics, float partialTicks, int mouseX, int mouseY) {
        ...
        slots.stream().filter(Slot::isActive).forEach(s -> Services.PLATFORM.drawSlotBackground(s, guiGraphics, this));
    ...
    protected void renderLabels(GuiGraphics guiGraphics, int p1, int p2) {
        if (menu.slots.isEmpty())
            guiGraphics.drawString(font, title, imageWidth / 2 - font.width(title.getString()) / 2, titleLabelY + 19 - topPos, Constants.ORANGE.getIntColor());
        else
            guiGraphics.drawString(font, title, imageWidth / 2 - font.width(title.getString()) / 2, titleLabelY, Constants.ORANGE.getIntColor());
    }
